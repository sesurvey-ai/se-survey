...
// (ตาราง kKfkInsurers ถอดออก 16/09/69 — ติ๊ก KFK ไม่ได้ใช้แล้วทั้งเว็บและแอป)
...
// ── ตรวจเลขบัตรตาม "ชนิดบัตรที่คนเลือก" (ติ๊กไทย / ต่างชาติ) — ไม่คำนวณชนิดจากตัวเลข ──
// ชุดเดียวกับเว็บ (RecordEditors.cidIssue) และตัวดึงงาน v1.1.27 (user เคาะ 21/09/69) · ขอให้บังคับบนแอปด้วย 21/09/69
// · ว่าง / "รอตรวจสอบ" / "-" = ไม่ทราบ ไม่ใช่เลขผิด (กติกาช่องข้อความบังคับของ EMCS)
// · ยาวเกิน 13 = ผิดทุกชนิด (ช่อง EMCS 13 ตัว · ไฟล์ปัดตกทั้งไฟล์) — พิมพ์ในแอปไม่ได้อยู่แล้ว แต่ draft/นำเข้าอาจมี
// · คนไทย = ตัวเลข 13 หลัก + หลักตรวจสอบ (mod 11)
// · ต่างชาติ = ผ่าน (พาสปอร์ต/บัตรต่างด้าวไม่มีสูตร) ยกเว้นช่องผู้บาดเจ็บที่ฟอร์ม EMCS ตรวจเอง (CheckTextBoxCitizenValid):
//   เลขล้วน/อักษรล้วนที่สั้นกว่า 13 ไม่รับ · ครบ 13 ตัวต้องผ่านหลักตรวจสอบ → ไม่มีบัตรไทยให้ใส่ "-"
// คืน '' = ผ่าน · ไม่ว่าง = เหตุผล (โชว์ใต้ช่อง + กั้นบันทึก/ส่งงาน)
String cidIssue(String cid, {required bool thai, bool injured = false}) {
  final v = cid.trim();
  if (v.isEmpty || v == 'รอตรวจสอบ' || RegExp(r'^-+$').hasMatch(v)) return '';
  if (v.length > 13) return 'เลขบัตรยาว ${v.length} ตัว — EMCS รับ 13 ตัว ตรวจกับบัตรอีกครั้ง';
  final digitsOnly = RegExp(r'^\d+$').hasMatch(v);
  if (thai) {
    if (!digitsOnly) return 'บัตรไทยต้องเป็นตัวเลข 13 หลัก — พาสปอร์ต/บัตรต่างชาติให้เอาติ๊ก "ไทย" ออก';
  ...
  if (injured) {
    if (v.length < 13 && (digitsOnly || RegExp(r'^[A-Za-z]+$').hasMatch(v))) {
      return 'EMCS ไม่รับเลขล้วน/อักษรล้วนที่สั้นกว่า 13 ตัวในช่องผู้บาดเจ็บ — ไม่มีบัตรไทยให้ใส่ "-"';
    }
    if (v.length == 13 && !cidChecksum(v)) return 'ครบ 13 ตัว EMCS ตรวจหลักตรวจสอบ — เลขนี้ไม่ผ่าน บัตรต่างชาติให้ใส่ "-"';
  }
  return '';
}
