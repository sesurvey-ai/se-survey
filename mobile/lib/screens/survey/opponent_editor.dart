...
class _OpponentEditorState extends State<OpponentEditor> {
  ...
  // ชุดค่า user เคาะ 17/09/69 (แทนชุด 16/09 ที่ใส่ "รอตรวจสอบ" ทุกช่องข้อความ + เก๋งเอเชีย/-ALL- + ประกัน อื่นๆ + วันเกิดปี 2525):
  //   เจ้าของรถ "-" · ประเภทรถ รถอื่นๆ · ทะเบียน "00" · จังหวัด อื่นๆ · เพศ ชาย + ชื่อ "ไม่ทราบชื่อ"
  //   (ไม่ใส่คำนำหน้า/นามสกุล → EMCS เห็น "ไม่ทราบชื่อ" คำเดียว) · วันเกิด 01/01/2500 + อายุจากปี พ.ศ. ·
  //   มีประกันภัยที่ ไม่มีบริษัทประกันภัย · กรมธรรม์ "-"
  // ช่องที่ไม่เติม (ที่อยู่ · เลขบัตร · โทร · ใบขับขี่) ปล่อยว่าง → บอทใส่ "-" ให้ EMCS ตามกติกาช่องข้อความบังคับ
  //   (โทร: ช่องรับเฉพาะตัวเลข ใส่ข้อความก็ถูกกรองทิ้ง) · ที่อยู่ว่าง = จังหวัด/อำเภอ/ตำบล ไม่บังคับ
  // ยี่ห้อ: รถอื่นๆ ไม่มี "-ALL-" และ EMCS ไม่บังคับยี่ห้อคู่กรณี (vlidOpoCar ไม่ตรวจ ddlCmfg — ยืนยันจากหน้า EMCS จริง 17/09/69)
  //   → ปล่อยว่าง บอท v1.1.7 ข้ามให้ · ถ้าช่างเลือกประเภทรถอื่นไว้แล้วแต่ยี่ห้อว่าง → "-ALL-" (ตัวเลือกจริงของ EMCS)
  void _applyPending() {
    void fill(String k, String v) { if (_ctl(k).text.trim().isEmpty) _ctl(k).text = v; }
    fill('owner_name', '-');
    fill('plate', '00');
    fill('first_name', 'ไม่ทราบชื่อ');
    ...
    if (_insurer.isEmpty) _insurer = 'ไม่มีบริษัทประกันภัย';   // _collect ส่งกรมธรรม์ "-" + เคลมที่/ประเภทกรมธรรม์ว่างให้เองเมื่อไม่มีประกัน
    fill('policy_no', '-');
