...
const assignCaseSchema = z.object({
   ...
   * บังคับเลือกทุกครั้ง (user สั่ง 15/09/69 — เดิม optional ให้ช่างเลือกเองบนแอป) · หน้าเว็บกันไว้ชั้นแรก ที่นี่กันซ้ำ
   */
  claim_type: z.enum(['F', 'D', 'A', 'C'], { errorMap: () => ({ message: 'ต้องเลือกประเภทเคลมก่อนมอบหมายงาน' }) }),
});
