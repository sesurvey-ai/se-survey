...
/**
 * อายุเก็บรูป (user เคาะ 24/09/69): **รูปลงเวลาเข้า-ออกงาน (att_*) เก็บ 1 เดือน** · รูปเคส 2 ปี
 *  - รูปเคส (รวมรูปถึงที่เกิดเหตุซึ่งอยู่ในโฟลเดอร์เคส) ยังไม่มีใบไหนถึง 2 ปี — เก่าสุด มิ.ย. 69
 *    → ตัวลบรูปเคสยังไม่ทำ ⚠️ ต้องทำก่อน มิ.ย. 71
 ...
 *  - แถวที่ลบรูปแล้ว → check_in_photo = NULL (หน้าตารางลงเวลา/บอร์ดเข้างาน/แอป แสดงแบบไม่มีรูปอยู่แล้ว)
 ...
 */
export const ATTENDANCE_PHOTO_DAYS = 30;
...
export async function purgeAttendancePhotos(opts: { apply: boolean }): Promise<RetentionStats> {
  const { rows } = await db.query(
    `SELECT id, check_in_photo FROM attendance_records
      WHERE check_in_photo IS NOT NULL
        AND check_in_at < ${BKK} - INTERVAL '${ATTENDANCE_PHOTO_DAYS} days'
  ...
  const cutoffMs = Date.now() - ATTENDANCE_PHOTO_DAYS * 24 * 60 * 60 * 1000;
  const orphans = expiredOrphans(await storage.listRootFiles('att_'), referenced, cutoffMs);
  ...
  try {
    ...
    if (!apply) {
      if (s.expired + s.orphans > 0) {
        console.log(`[photoRetention] ปิดอยู่ (รายงานอย่างเดียว): รูปลงเวลาเกิน ${ATTENDANCE_PHOTO_DAYS} วัน ${s.expired} รูป`
          + ` + ไฟล์ไม่มีแถวอ้างถึง ${s.orphans} ไฟล์ — ตั้ง PHOTO_RETENTION_ENABLED=1 เพื่อลบจริง`);
      }
      return;
    }
    if (s.deleted + s.failed > 0) {
      console.log(`[photoRetention] ลบรูปลงเวลาเกิน ${ATTENDANCE_PHOTO_DAYS} วัน: ${s.deleted} ไฟล์`
